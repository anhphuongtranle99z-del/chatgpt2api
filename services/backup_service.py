...
def _openssl_encrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        raise BackupError("Môi trường hiện tại thiếu openssl, không thể tạo bản sao lưu mã hóa") from exc
    except subprocess.CalledProcessError as exc:
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Mã hóa bản sao lưu thất bại: {detail or 'openssl chạy thất bại'}") from exc
...
def _openssl_decrypt(data: bytes, passphrase: str) -> bytes:
    ...
    try:
        ...
        detail = (exc.stderr or b"").decode("utf-8", errors="replace").strip()
        raise BackupError(f"Giải mã bản sao lưu thất bại: {detail or 'openssl chạy thất bại'}") from exc
    return result.stdout
...
class CloudflareR2Client:
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise BackupError(f"Cấu hình R2 chưa đầy đủ: thiếu {', '.join(missing)}")
    ...
    def upload_bytes(self, key: str, payload: bytes, *, content_type: str, metadata: dict[str, str] | None = None) -> dict[str, object]:
        ...
        if response.status_code >= 400:
            raise BackupError(f"Tải bản sao lưu lên thất bại: HTTP {response.status_code}")
...
class BackupService:
    ...
    def delete_backup(self, key: str) -> None:
        ...
        if not candidate:
            raise BackupError("Key đối tượng sao lưu không được để trống")
        client = CloudflareR2Client(config.get_backup_settings())
        try:
    ...
    def download_backup(self, key: str) -> dict[str, object]:
        ...
        if candidate.endswith(".enc"):
            ...
            if not passphrase:
                raise BackupError("Chưa cấu hình mật khẩu mã hóa, không thể tải xuống và giải mã bản sao lưu đã mã hóa")
    ...
    def run_backup(self, *, trigger: str = "manual") -> dict[str, object]:
        with self._lock:
            ...
            if self._running:
                raise BackupError("Đang có tác vụ sao lưu chạy")
            started_at = _iso_now()
            self._running = True
            save_backup_state({
    ...
    def _decode_backup_payload(self, key: str, payload: bytes) -> dict[str, object]:
        ...
        if key.endswith(".enc"):
            passphrase = _clean(config.get_backup_settings().get("passphrase"))
            if not passphrase:
                raise BackupError("Chưa cấu hình mật khẩu mã hóa, không thể xem bản sao lưu đã mã hóa")
    ...
    def _decode_archive_detail(self, payload: bytes) -> dict[str, object]:
        ...
        try:
            ...
            raise BackupError("Không phân tích được gói nén sao lưu, bản sao lưu có thể đã hỏng") from exc
