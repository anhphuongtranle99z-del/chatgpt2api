...
class LoggedCall:
    ...
    async def run(self, handler, *args, sse: str = "openai"):
        ...
        try:
            ...
            self.log(" thất bại", status="failed", error=str(exc))
            return _image_error_response(exc)
        except HTTPException as exc:
            self.log(" thất bại", status="failed", error=str(exc.detail))
            raise
    ...
    def stream(self, items):
        ...
        try:
            ...
            failed = True
            self.log(" luồng thất bại", status="failed", error=str(exc), urls=urls)
            raise
        finally:
            if not failed:
                self.log(" kết thúc luồng", urls=urls)
