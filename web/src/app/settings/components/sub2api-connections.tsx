...
export function Sub2APIConnections() {
    ...
    pollTimerRef.current = window.setInterval(() => {
      void fetchSub2APIServers()
        ...
        .catch((error) => {
          ...
          toast.error(error instanceof Error ? error.message : "Không kiểm tra được tiến độ nhập");
        });
    }, 1500);
  ...
  const handleFetchGroups = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Không lấy được nhóm");
    } finally {
  ...
  const handleStartImport = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Không khởi động được tác vụ nhập");
    } finally {
        ...
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">
                  Sau khi cấu hình máy chủ Sub2API, bạn có thể truy vấn tài khoản OpenAI OAuth và nhập hàng loạt vào pool cục bộ.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-stone-400">Nhấn "Thêm kết nối" để lưu thông tin Sub2API.</p>
              </div>
            </div>
          ) : (
                          ...
                          return (
                            <div className="rounded-lg border border-stone-200 bg-white px-3 py-3">
                              ...
                              <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                                ...
                                <span>Lỗi {importJob.failed}</span>
                              </div>
                            </div>
                          );
                        })()}
          ...
          <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
            ...
            <ul className="mt-1 list-inside list-disc space-y-0.5">
              <li>Nhập địa chỉ Sub2API và tài khoản quản trị viên (hoặc Admin API Key), rồi lưu thành một kết nối.</li>
              <li>Nhấn "Đồng bộ" trên một kết nối để lấy danh sách tài khoản có platform=openai và type=oauth.</li>
              <li>Sau khi chọn các tài khoản cần dùng, backend sẽ lấy access_token song song, tự nhập vào pool cục bộ và làm mới trạng thái.</li>
              <li>Chỉ đọc access_token trong credential của sub2api; các trường như refresh_token sẽ không được ghi vào cục bộ.</li>
            </ul>
          </div>
        </CardContent>
      </Card>
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {editingServer ? "Cập nhật thông tin kết nối Sub2API" : "Thêm kết nối Sub2API mới"}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              {remoteGroups && remoteGroups.length > 0 ? (
                ...
                <Input
                  ...
                  placeholder="Để trống để đồng bộ mọi nhóm; hoặc nhập ID nhóm / ungrouped"
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              )}
              {editingServer ? (
                ...
                <div className="text-xs text-stone-500">
                  Sau khi thêm kết nối, mở hộp thoại chỉnh sửa và chọn "Lấy nhóm" để chọn nhóm cụ thể.
                </div>
              )}
            </div>
          </div>
        ...
        <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between text-sm text-stone-500">
            <span>
              ...
              {Math.min(safeAccountPage * currentPageSize, filteredAccounts.length)} trong tổng số {filteredAccounts.length} mục
            </span>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
