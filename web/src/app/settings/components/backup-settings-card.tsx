...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "logs", label: "Nhật ký điều phối và gọi API" },
  ...
  { key: "images", label: "Thư mục file ảnh" },
];
...
export function BackupSettingsCard() {
  ...
  const handleDownload = async (key: string, name: string) => {
    try {
      ...
      if (!authKey) {
        toast.error("Phiên đăng nhập hiện tại đã hết hạn, vui lòng đăng nhập lại rồi tải xuống");
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          message = response.status === 401 ? "Phiên đăng nhập đã hết hạn, vui lòng đăng nhập lại rồi thử lại" : message;
        }
    ...
    <>
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Sao lưu định kỳ dữ liệu quan trọng lên Cloudflare R2; hỗ trợ mã hóa tùy chọn, luân phiên, chạy thủ công và dọn lịch sử.</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={backupState?.running ? "warning" : backupState?.last_status === "success" ? "success" : "secondary"} className="rounded-md">
                {backupState?.running ? "Đang sao lưu" : backupState?.last_status === "success" ? "Lần gần nhất thành công" : backupState?.last_status === "error" ? "Lần gần nhất thất bại" : "Chưa chạy"}
              </Badge>
            </div>
          </div>

          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Tài khoản và khóa người dùng sẽ được xuất thành snapshot logic từ backend lưu trữ hiện tại, không phụ thuộc tầng dưới là `json`, `sqlite`, `postgres` hay `git`. Mặc định không sao lưu thư mục ảnh để tránh kích thước sao lưu quá lớn.
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Khoảng cách sao lưu định kỳ</label>
            ...
            <p className="text-xs text-stone-500">Đơn vị là phút; sau khi dịch vụ khởi động sẽ tự động chạy theo khoảng cách này.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Sau khi tải lên thành công, tự động xóa các bản sao lưu cũ hơn. Nhập `0` để không tự luân phiên.</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Chỉ dùng khi bật mã hóa. Hãy lưu giữ cẩn thận, nếu không sẽ không thể giải mã nội dung sao lưu.</p>
          </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500">Chọn dữ liệu cần đưa vào gói sao lưu theo từng thành phần.</p>
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-stone-500">Có thể xem thông tin đối tượng và xóa trực tiếp bản sao lưu từ xa.</p>
            </div>
          </div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Chưa có bản sao lưu từ xa. Lưu cấu hình và chạy sao lưu thủ công một lần để hiển thị tại đây.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Tạm thời không đọc được chi tiết sao lưu; nếu đây là bản sao lưu đã mã hóa, hãy xác nhận đã nhập đúng mật khẩu mã hóa hiện tại và lưu cấu hình trước.
              </div>
            ) : (
              <>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Nội dung snapshot</h4>
