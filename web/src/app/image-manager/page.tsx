...
function ImageManagerContent() {
  ...
  const handleSetTags = async (item: ManagedImage, tags: string[]) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Cập nhật thẻ thất bại");
    }
  };
  ...
  const handleDeleteTag = async (tag: string) => {
    try {
      ...
      toast.success(`Đã xóa thẻ "${tag}" khỏi ${result.removed_from} ảnh`);
    } catch (error) {
    ...
    <section className="space-y-5">
      ...
      <Dialog open={Boolean(deleteMode)} onOpenChange={(open) => (!open ? setDeleteMode(null) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          ...
          <p className="text-sm text-stone-600">
            Xác nhận xóa {selectedCount} ảnh? Sau khi xóa không thể khôi phục.
          </p>
          <DialogFooter>
      ...
      <Dialog open={Boolean(tagDeleteTarget)} onOpenChange={(open) => { if (!open) setTagDeleteTarget(null); }}>
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <p className="text-sm text-stone-600">
            Bạn có chắc muốn xóa thẻ <span className="font-semibold">"{tagDeleteTarget}"</span> không? Thẻ này sẽ bị gỡ khỏi tất cả ảnh.
          </p>
          <DialogFooter>
