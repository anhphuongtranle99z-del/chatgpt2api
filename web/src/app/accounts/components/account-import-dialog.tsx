...
function readFileAsText(file: File) {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error ?? new Error(`Không thể đọc file: ${file.name}`));
...
export function AccountImportDialog({ disabled, onImported }: AccountImportDialogProps) {
  ...
  const submitTokens = async (tokens: string[], successText?: string) => {
    ...
    try {
      ...
      if ((data.errors?.length ?? 0) > 0) {
        ...
        toast.error(
          `${successText ?? "Hoàn tất nhập"}, đã thêm ${data.added ?? 0}, đã làm mới ${data.refreshed ?? 0}, thất bại ${data.errors?.length ?? 0}${firstError ? `, lỗi đầu tiên: ${firstError}` : ""}`,
        );
      } else {
        toast.success(
          `${successText ?? "Hoàn tất nhập"}, đã thêm ${data.added ?? 0}, bỏ qua ${data.skipped ?? 0} mục trùng lặp, đã tự động làm mới thông tin tài khoản`,
        );
      }
    } catch (error) {
  ...
  const handleImportTokenText = async () => {
    await submitTokens(splitTokens(tokenInput), "Hoàn tất nhập Access Token");
  };
  ...
  const handleImportSessionJson = async () => {
    ...
    try {
      ...
      await submitTokens([token], "Hoàn tất nhập Session JSON");
    } catch (error) {
  ...
  const handleCpaSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsedFileCount === 0) {
        toast.error("Không đọc được access_token dùng được trong các file CPA JSON này");
        return;
      }
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="space-y-1">
                ...
                <div className="text-sm leading-6 text-stone-500">Hỗ trợ `.txt`; nội dung file cũng là mỗi dòng một Token.</div>
              </div>
              <Button
                type="button"
                variant="outline"
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4 text-sm leading-6 text-stone-600">
            ...
            , sao chép JSON đầy đủ trả về từ trang; hệ thống sẽ tự trích xuất `accessToken` để nhập.
          </div>
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            <div className="font-medium">Cảnh báo rủi ro</div>
            <div>
              Không dùng tài khoản chính của bạn; nên dùng tài khoản phụ ít sử dụng để nhập, tránh nguy cơ bị khóa tài khoản. Dự án này không chịu trách nhiệm về rủi ro khóa tài khoản.
            </div>
          </div>
          <div className="space-y-2">
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50 p-5">
            <div className="space-y-2">
              <div className="text-sm font-medium text-stone-800">Chọn nhiều file CPA JSON cục bộ</div>
              <div className="text-sm leading-6 text-stone-500">
                Mỗi file phải là một đối tượng JSON. Hệ thống sẽ tự trích xuất `access_token` hoặc `accessToken`,
              </div>
            </div>
            <Button
              type="button"
          ...
          {pendingCpaImport ? (
            <div className="rounded-2xl border border-stone-200 bg-white p-4 text-sm leading-6 text-stone-600">
              Lần gần nhất đã đọc được {pendingCpaImport.parsedFileCount} Token
              {pendingCpaImport.errorCount > 0 ? `, có ${pendingCpaImport.errorCount} file chưa trích xuất thành công` : ""}.
            </div>
          ) : null}
        </div>
      );
    ...
    return (
      <div className="space-y-3">
        <MethodCard
          ...
          description="Hỗ trợ dán trực tiếp, mỗi dòng một Token; cũng có thể đọc từ file TXT."
          icon={KeyRound}
          onClick={() => setMethod("token")}
        />
        <MethodCard
          title="Nhập Session JSON"
          description="Sao chép JSON đầy đủ từ API session của chatgpt.com, tự động trích xuất accessToken."
          icon={FileJson}
          onClick={() => setMethod("session")}
        />
        <MethodCard
          title="Nhập file CPA JSON"
          description="Hỗ trợ chọn nhiều file JSON cục bộ cùng lúc, đọc access_token trong từng đối tượng rồi nhập."
          icon={Files}
          onClick={() => setMethod("cpa")}
        />
        <MethodCard
          title="Nhập từ máy chủ CPA từ xa"
          description="Sang trang cài đặt để cấu hình máy chủ CPA từ xa rồi thực hiện nhập."
          icon={Files}
          onClick={() => {
            setOpen(false);
            resetState();
        ...
        <MethodCard
          ...
          description="Sang trang cài đặt để cấu hình máy chủ Sub2API, rồi chọn tài khoản OpenAI trong đó để nhập."
          icon={ServerCog}
          onClick={() => {
            setOpen(false);
            resetState();
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {method === "menu"
                ? "Chọn một cách nhập. Sau khi nhập thành công, hệ thống sẽ tự động lấy email, loại và hạn mức."
                : method === "token"
                  ? "Hỗ trợ dán thủ công hoặc nhập từ file TXT, mỗi dòng một Token."
                  : method === "session"
                    ? "Dán Session JSON đầy đủ, hệ thống sẽ tự trích xuất accessToken."
                    : "Hỗ trợ đọc nhiều file JSON cục bộ cùng lúc và xác nhận số lượng trước khi gửi."}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              {pendingCpaImport
                ? `Đã nhận diện ${pendingCpaImport.parsedFileCount} Token. Xác nhận nhập?`
                : "Chưa đọc được Token nào có thể nhập."}
              {pendingCpaImport?.errorCount
                ? ` Có ${pendingCpaImport.errorCount} file chưa trích xuất thành công.`
                : ""}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="pt-2">
            ...
            <Button
              className="h-10 rounded-xl bg-stone-950 px-5 text-white hover:bg-stone-800"
              onClick={() => void submitTokens(pendingCpaImport?.tokens ?? [], "Hoàn tất nhập CPA JSON")}
