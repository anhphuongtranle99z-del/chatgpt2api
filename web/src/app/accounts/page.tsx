...
> = {
  ...
  禁用: { label: "Đã tắt", icon: Ban, badge: "secondary" },
};

const metricCards = [
  { key: "total", label: "Tổng tài khoản", color: "text-stone-900", icon: UserRound },
  { key: "active", label: "Bình thường", color: "text-emerald-600", icon: CheckCircle2 },
  ...
  { key: "disabled", label: "Đã tắt", color: "text-stone-500", icon: Ban },
...
function AccountsPageContent() {
  ...
  const handleRefreshAccounts = async (accessTokens: string[]) => {
    if (accessTokens.length === 0) {
      toast.error("Không có tài khoản cần làm mới");
      return;
    }
    ...
    try {
      ...
      if (data.errors.length > 0) {
        ...
        toast.success(`Đã làm mới ${data.refreshed} tài khoản`);
      }
    } catch (error) {
    ...
    <>
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Quản lý kho tài khoản</h1>
        </div>
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Chỉnh trạng thái tài khoản thủ công.
            </DialogDescription>
          </DialogHeader>
          ...
          <CardContent className="space-y-0 p-0">
            ...
            <div className="border-t border-stone-100 px-4 py-4">
              <div className="flex items-center justify-center gap-3 overflow-x-auto whitespace-nowrap">
                ...
                Hiển thị mục {filteredAccounts.length === 0 ? 0 : startIndex + 1} -{" "}
                {Math.min(startIndex + Number(pageSize), filteredAccounts.length)}, tổng{" "}
