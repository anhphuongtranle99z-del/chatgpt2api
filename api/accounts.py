...
def create_router() -> APIRouter:
    ...
    async def update_user_key(
        ...
        if not updates:
            raise HTTPException(status_code=400, detail={"error": "Chưa phát hiện thay đổi, hãy chỉnh sửa rồi lưu lại"})
        try:
            item = auth_service.update_key(key_id, updates, role="user")
        except ValueError as exc:
            raise HTTPException(status_code=400, detail={"error": str(exc)}) from exc
        if item is None:
            raise HTTPException(status_code=404, detail={"error": "Khóa người dùng này không tồn tại hoặc đã bị xóa"})
